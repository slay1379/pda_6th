...
지금 실행한 /api/calc는 매우 짧은 CPU 바운드 작업
이런 단기적 테스트에서는 t2의 크레딧을 다 쓰기 전에 테스트가 끝남 → 평균 응답 시간이 더 낮음
장기 테스트(예: 10분 이상)나 실서비스 시나리오에선 보통 t3가 더 낫습니다.
t2는 크레딧 소진 시 성능 급락 vs t3는 지속 부하에도 일정 수준 성능 유지 (과금 조건 포함)
